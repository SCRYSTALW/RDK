...
soundfile=['correct.wav','correct.wav','wrong3.wav']
...
expName = '01 RDK RIGHT '  # from the Builder filename that created this script
expInfo = {
    'participant': '',
    'session': '001',
}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/sc/Desktop/Psychopy/Psychopy staircase/01 RDK RIGHT _lastrun.py',
...
dots = visual.DotStim(
    ...
    signalDots='same', noiseDots='direction',dotLife=2.0,
...
conditions = data.importConditions('Coherence periphery old.xlsx')
...
for level, condition in trials:
    ...
    for thisComponent in blockComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials.addOtherData('fixation_2.started', fixation_2.tStartRefresh)
    trials.addOtherData('fixation_2.stopped', fixation_2.tStopRefresh)
    trials.addOtherData('blank500.started', blank500.tStartRefresh)
    trials.addOtherData('blank500.stopped', blank500.tStopRefresh)
    ...
    while continueRoutine:
        ...
        if key_resp.status == STARTED and not waitOnFlip:
            ...
            if len(_key_resp_allKeys):
                ...
                # was this correct?
                if (key_resp.keys == str(qp_key)) or (key_resp.keys == qp_key):
                    key_resp.corr = 1
                else:
                    key_resp.corr = 0
                # a response ends the routine
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials.addOtherData('dots.started', dots.tStartRefresh)
    trials.addOtherData('dots.stopped', dots.tStopRefresh)
    # check responses
    if key_resp.keys in ['', [], None]:  # No response was made
        key_resp.keys = None
        # was no response the correct answer?!
        if str(qp_key).lower() == 'none':
    ...
    if key_resp.corr:  # correct response
        ...
        incorrect_count += 1
        if coherence == 1 and incorrect_count >= 5:  # five or more incorrect responses
            # end the experiment and display a message
            print("Too many incorrect responses. Ending experiment.")
            core.quit()
        elif coherence == 1 and incorrect_count >= 3:  # three or more incorrect responses
            sound = soundfile[2]
        else:
            sound = soundfile[1]
    
    sound_1.setSound('correct.wav', secs=0.5, hamming=True)
